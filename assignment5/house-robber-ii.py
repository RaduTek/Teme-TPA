class Solution:
    ...
    def calcRobber(self, house: int):
        ...
        vals = [0] * 3
        vals[0] = self.calcRobber(house - 2) + myValue
        # if we pick the previous house
        vals[1] = self.calcRobber(house - 1)
    ...
    def robMe(self, nums: List[int]) -> int:
        self.nums = nums
        # Init array of calculated house values
        # -1 means value not calculated yet
        self.value = [-1] * len(nums)

        return self.calcRobber(len(self.nums) - 1)

    def rob(self, nums: List[int]) -> int:
        # Handle small number count cases
        if len(nums) == 1:
            return nums[0]
        if len(nums) == 2:
            return max(nums)

        # Rob from the 1st house to the 2nd to last one
        calc1 = self.robMe(nums[:-1])
        # Rob from the 2nd house to the last one
        calc2 = self.robMe(nums[1:])
